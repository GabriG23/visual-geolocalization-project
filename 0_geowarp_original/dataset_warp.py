...
class HomographyDataset(torch.utils.data.Dataset):                                      # crea il dataset per l'omografia
    def __init__(self, root_path, k=0.1, is_debugging=False):
        super().__init__()
        self.images_paths = sorted(glob(f"{root_path}/**/*.jpg", recursive=True))       # riceve il path e si salva tutti i path
        self.k = k                                                                      # indica quanto è difficile generare la coppia omografica
        self.is_debugging = is_debugging
    
    def __getitem__(self, index):
        image_path = self.images_paths[index]
        source_img = datasets_util.open_image_and_apply_transform(image_path)                       # trasforma l'immagine in un tensore, specifiche in datasets_util.py
