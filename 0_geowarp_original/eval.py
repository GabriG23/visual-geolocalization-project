...
if __name__ == "__main__":
    ...
    ############### MODEL ###############
    features_extractor = network.FeaturesExtractor(args.arch, args.pooling)                     # crea l'encoder
    global_features_dim = commons.get_output_dim(features_extractor, args.pooling)              # prende la corrispettiva dimensione
    homography_regression = network.HomographyRegression(kernel_sizes=args.kernel_sizes, channels=args.channels, padding=1)     # crea il warping regression module
    ...
    model = network.Network(features_extractor, homography_regression).cuda().eval()             # mette il modello in evaluation 
    model = torch.nn.DataParallel(model)                                                         # parallelizza l'applicazione
    ############### MODEL ###############
    
    ############### DATASETS & DATALOADERS ###############
    geoloc_test_dataset = dataset_geoloc.GeolocDataset(args.datasets_folder, args.dataset_name, split="test", positive_dist_threshold=args.positive_dist_threshold)
    # prende il dataset in modalità test
    logging.info(f"Geoloc test set: {geoloc_test_dataset}")
    ############### DATASETS & DATALOADERS ###############
    
    ############### TEST ###############
    # calcola le recall e fa reranking
