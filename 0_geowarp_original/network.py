...
class GeM(nn.Module):
    ...
    def forward(self, x):
        ...
        x = x.reshape(B, C)                 # fa il reshape di x, cosa che non fa in cospalce
        return x
...
def gem(x, p=3, eps=1e-6):  # uguale a cosplace
...
def feature_L2_norm(feature):
    ...
    # credo faccia le stess cose ma in modi diversi
    # Forward cosplace
    # def forward(self, x):
...
class FeaturesExtractor(torch.nn.Module):
    ...
    def __init__(self, arch, pooling):
        super().__init__()
        if arch == "resnet50":                          # sceglie l'architettura
            model = torchvision.models.resnet50(pretrained=True)
            layers = list(model.children())[:-3]  # rimuove gli ultimi tre layers della backbone (avg pooling and FC layer) in modo
        elif arch == "vgg16":
        ...
        if pooling == "netvlad":                                # sceglie il pooling
            encoder_dim = commons.get_output_dim(self.encoder)
            self.pool = NetVLAD(dim=encoder_dim)
        elif pooling == "gem":
            self.pool = nn.Sequential(L2Norm(), GeM())          # cosplace ha in più flatten, nn.linear e un'altra L2norm

    def forward(self, x, f_type="local"):
        x = self.encoder(x)                     # esegue l'encoder
        if f_type == "local":                   # descrittori locali avgpool e poi l2norm
            x = self.avgpool(x)
            return self.l2norm(x)
        elif f_type == "global":                # descrittori globali gem o netvlad
            return self.pool(x)
        else:
