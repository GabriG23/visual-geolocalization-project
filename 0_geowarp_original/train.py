...
if __name__ == "__main__":
    ...
    ############### MODEL ###############
    features_extractor = network.FeaturesExtractor(args.arch, args.pooling)   # arch alexnet, vgg16 o resnet50, pooling netvlad o gem
    global_features_dim = commons.get_output_dim(features_extractor, args.pooling) # dimensione degli encoder
    # feature_extractor è il nostro ENCODER che viene usato più volte, per global feature extraction, warping module e similarity score computation
    if args.resume_fe is not None:
        state_dict = torch.load(args.resume_fe)   #ricarica lo stato
    ...
    # prende il train
    geoloc_train_dataset = dataset_geoloc.GeolocDataset(args.datasets_folder, args.dataset_name, split="train", positive_dist_threshold=args.positive_dist_threshold)  # dataset per il train

    logging.info(f"Geoloc train set: {geoloc_train_dataset}")

    geoloc_test_dataset = dataset_geoloc.GeolocDataset(args.datasets_folder, args.dataset_name, split="test",  positive_dist_threshold=args.positive_dist_threshold)   # dataset per il test
    logging.info(f"Geoloc test set: {geoloc_test_dataset}")

    # dataset per il warping dallo stesso dataset di train
    ss_dataset = dataset_warp.HomographyDataset(root_path=f"{args.datasets_folder}/{args.dataset_name}/images/train", k=args.k)                                           
    # crea il dataloader per il warping dataset
    ss_dataloader = commons.InfiniteDataLoader(ss_dataset, shuffle=True, batch_size=args.batch_size_ss, num_workers=args.ss_num_workers, pin_memory=True, drop_last=True)
    ss_data_iter = iter(ss_dataloader)   # crea iteratore sui data loader
    
    if args.consistency_w != 0 or args.features_wise_w != 0:                 # fa alcuni check
        # crea il dataset delle query predictions con il dataset di training
        dataset_qp = dataset_qp.DatasetQP(model, global_features_dim, geoloc_train_dataset, qp_threshold=args.qp_threshold)        
        # crea il loader
        dataloader_qp = commons.InfiniteDataLoader(dataset_qp, shuffle=True, batch_size=max(args.batch_size_consistency, args.batch_size_features_wise), num_workers=args.qp_num_workers, pin_memory=True, drop_last=True)
        data_iter_qp = iter(dataloader_qp)  # crea l'iter
    ############### DATASETS & DATALOADERS ###############
    
    ############### LOSS & OPTIMIZER ###############
    mse = torch.nn.MSELoss()  # criterio per la loss, cosplace usa crossEntropy loss. MSE è un criterio che misura the mean squared error tra gli elementi in input x e il target y
    if args.optim == "adam":
    ...
    ############### TRAIN ###############
    for epoch in range(args.n_epochs):                                # itera sulle epoch (noi ne abbiamo 3)
        
        homography_regression = homography_regression.train()         # inizia il train dell'homography_regression # WARPING REGRESSION MODULE
        epoch_losses = np.zeros((0, 3), dtype=np.float32)             # inizializza il vettore delle loss
        
        for iteration in tqdm(range(args.iterations_per_epoch), desc=f"Train epoch {epoch}", ncols=100):  # ncols è la lunghezza della barra, fa 50k iterazioni
            # dal warping dataset prende le due immagini warped e i due punti delle intersezioni
            warped_img_1, warped_img_2, warped_intersection_points_1, warped_intersection_points_2 = to_cuda(next(ss_data_iter))  # warping dataset
            # Iq, Ip, tq e tp
            if args.consistency_w != 0 or args.features_wise_w != 0:
                queries, positives = to_cuda(next(data_iter_qp))    # prende prossimo item dal dataset prediction
            ...
            with torch.no_grad():                          # no gradient
                ...
                if args.consistency_w != 0:
                    queries_cons = queries[:args.batch_size_consistency]                                                 # prende la conistency = 16                                          
                    positives_cons = positives[:args.batch_size_consistency]                                             # anche qui = 16
            ...
            optim.zero_grad()   # setta il gradiente a zero per evitare double counting (passaggio classico dopo ogni iterazione)
            
            # ss_loss  # self supervised loss          # calcola la prima loss, a noi serve??
            if args.ss_w != 0:
            ...
            # features_wise_loss     # assicura che le feature siano più vicine possibile dopo che vengono estratte dal training
    ...
    # LA PARTE CHE MI INTERESSA STA QUI, DOBBIAMO AGGIUNGERE QUESTO PEZZO IN COSPLACE

    homography_regression = homography_regression.eval()  # equivale a self.train(false) mette la rete in evaluation node
    ...
    # geoloc_test_dataset contains images (queries and gallery)
    # model -> network.Network
    # global_feature_dim: dimension of feature 
    # return: recalls, recalls_pretty_str, predictions (an array containing the first 20 predictions for each query with shape [queries_num, 20]), correct_bool_mat, distances
    # a noi interessano le prime tre return
    logging.info(f"baseline test: {test_baseline_recalls_pretty_str}") # stampa le recall standard
    ...
    logging.info(f"test after warping - {reranked_test_recalls_pretty_str}") # stampa le recall warpate
    ############### TEST ###############
