...
if args.reduction:
    autoencoder_optimizer = SGD(model.autoencoder.parameters(), lr=3e-4, momentum=0.9)
...
attention_parameters = [*model.attn_classifier.parameters(), *model.attention.parameters()]
attention_optimizer = SGD(model.parameters(), lr=3e-4, momentum=0.9)
#### Datasets
    ...
    for iteration in tqdm(range(args.iterations_per_epoch), ncols=100):     # ncols è la grandezza della barra  
        ...
        if not args.use_amp16:
            ...
            if args.reduction:
                ...
                loss = global_loss + attn_loss + 10 * rec_loss 
            else:
