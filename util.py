...
def save_checkpoint_homography(state: dict, output_folder: str, ckpt_filename: str = "last_checkpoint.pth"):

    # TODO it would be better to move weights to cpu before saving
    checkpoint_path = f"{output_folder}/{ckpt_filename}"
    torch.save(state, checkpoint_path)
    torch.save(state["model_state_dict"], f"{output_folder}/best_model.pth")        # comprende sia encoder sia homography
    torch.save(state["homography_state_dict"], f"{output_folder}/homography.pth")

def resume_train_homography(args: Namespace, output_folder: str, model: torch.nn.Module, model_optimizer: Type[torch.optim.Optimizer], homography_regression: torch.nn.Module):
    logging.info(f"Loading checkpoint: {args.resume_train}")
    checkpoint = torch.load(args.resume_train)
    start_epoch_num = checkpoint["epoch_num"]
    model_state_dict = checkpoint["model_state_dict"]
    homography_regression_state_dict = checkpoint["homography_state_dict"]

    model.load_state_dict(model_state_dict)
    
    homography_regression.load_state_dict(homography_regression_state_dict)
    ...
    return model, model_optimizer, start_epoch_num
