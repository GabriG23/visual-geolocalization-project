...
model = network.GeoLocalizationNet(args.backbone, args.fc_output_dim, args.layers)      # istanzia il modello con backbone e dimensione del descrittore
...
for epoch_num in range(start_epoch_num, args.epochs_num):        # inizia il training
    ...
    logging.info(f"Epoch {epoch_num:02d} in {str(datetime.now() - epoch_start_time)[:-7]}, {val_ds}: {recalls_str}")  # [recalls_str:20]
