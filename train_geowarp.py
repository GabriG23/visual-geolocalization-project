    ...
    for iteration in tqdm(range(args.iterations_per_epoch), ncols=100):    # ncols è la grandezza della barra, 10k iterazioni per gruppo
        ...
        if not args.use_amp16:
            ...
            if args.ss_w != 0:  # ss_loss  # self supervised loss    guides the network to learn to estimate the points 
                ...
                ss_loss = (mse(pred_warped_intersection_points_1[:, :4], warped_intersection_points_1).float() +
                        mse(pred_warped_intersection_points_1[:, 4:], warped_intersection_points_2).float() +
                        mse(pred_warped_intersection_points_2[:, :4], warped_intersection_points_2).float() +
                        mse(pred_warped_intersection_points_2[:, 4:], warped_intersection_points_1).float())
                print(ss_loss)
                print(ss_loss.dtype)
