...
def cosine_sim(x1: torch.Tensor, x2: torch.Tensor, dim: int = 1, eps: float = 1e-8) -> torch.Tensor:
    ...
    ip = torch.mm(x1, x2.t())   #ho tolto una transpose                     # prodotto matriciale tra feature e weight, in questo caso i weight vengono trasposti
    
    w1 = torch.norm(x1, 2, dim)    # 32                  # normalizza il vettore feature, 2 = L2 norm. Ritorna una matrice norm o un vettore norm di un tensore                                          
                                                     #  dim serve per specificare quale dimensione/i del tensor di input calcolare (in questo caso dim = 1)
    w2 = torch.norm(x2, 2, dim)    # 5965                  # normalizza il vettore weight 
...
class MarginCosineProduct(nn.Module): # CosFace
    ...
    Args:
        ...
        m: margin
    """
    def __init__(self, in_features: int, out_features: int, s: float = 30.0, m: float = 0.40): # m >= 0, m = 0.5, 0.4, 0.3, in_features = 512, out_features=lunghezza del gruppo (classi)
        super().__init__()
    ...
    def forward(self, inputs: torch.Tensor, label: torch.Tensor) -> torch.Tensor:
        ...
        # inputs 32 5965
        # outputs 5965 224
        cosine = cosine_sim(inputs, self.weight)      # calcola la cosine similarity, cos(theta), vedi sopra, esce [32, 5965]
