...
def cosine_sim(x1: torch.Tensor, x2: torch.Tensor, dim: int = 1, eps: float = 1e-8) -> torch.Tensor:
    ...
    ip = torch.mm(x1, x2)   #ho tolto una transpose                     # prodotto matriciale tra feature e weight, in questo caso i weight vengono trasposti
    ...
    return x1 / torch.ger(w1, w2).clamp(min=eps)  # nel clamp metto solo il valore minimo 1e-8
